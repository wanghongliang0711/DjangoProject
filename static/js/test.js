$(function(){
     ...
     var chart = Highcharts.chart('contain', {
                            ...
                            subtitle: {
                                text: 'Show Info of trip'	// 副标题
                            },
                            chart: {
                                zoomType: 'x'	// 可以沿x轴放大，用鼠标拖动一段距离
                            },
                            xAxis:{
                                    ...
                                    events:{		// 动态函数
                                        click:function(e){		// 点击highchart里时触发
                                           ...
                                           huaxian(hx);			// 调huaxian函数把索引值传进去在highcharts上画标示线
                                            ...
                                            for(i=0,length=chart.series.length;i<length;i++){	// 开始遍历所有线
                                                if(chart.series[i].visible){					// 遍历到某条线时若该线可见的话
     ...
     var mytable = $('#example').DataTable({		//生成表格对象开始画表格
           ...
           aLengthMenu:[[1800,3600], [1800,3600]],	// 第一个[1800,3600]是表格每页显示1800/3600条数据受第二个控制,第二个表格下拉框一页显示1800/3600条数据？？
...
<!--                    折线图上滚动条滑动至该点-->
                    var startpindex = pointindex-200>0?pointindex-200:0;
                    var endpindex = pointindex+200>(pt.length)?(pt.length):pointindex+200;
